...
(spec `mmmzstpq` §6 / §9). It lives in-repo under `apps/telemetry/` and is deployed
...
`../desktop/tests/fixtures/telemetry/session_all_streams.json` and `crash_batch.json`
...
`apps/telemetry/` is committed; `.wrangler/` is gitignored.
...
8. Smoke-test against the §6.3 fixture body:
   ...
   curl -i -X POST https://pod.aaronmallen.dev/telemetry/v1/ingest \
     ...
     --data @../desktop/tests/fixtures/telemetry/session_all_streams.json   # → 204
   curl -i -X POST https://pod.aaronmallen.dev/telemetry/v1/ingest \
     -H 'Content-Type: application/json' \
     --data @../desktop/tests/fixtures/telemetry/session_all_streams.json   # → 401 (no key)
