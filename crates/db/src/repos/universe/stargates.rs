...
fn validate_records(records: &[Stargate]) -> Result<Vec<ActiveModel>, Error> {
...
mod tests {
  ...
  fn make_stargate(id: i32, name: &str) -> Stargate {
    Stargate::new(id, name)
  }
  ...
  mod all {
    ...
    #[tokio::test]
    async fn returns_empty_when_no_stargates() {
    ...
    async fn returns_all_after_upsert() {
      let db = setup_db().await;
      let repo = Repo::new(&db);
      repo.upsert(&make_stargate(1, "Gate A")).await.unwrap();
      repo.upsert(&make_stargate(2, "Gate B")).await.unwrap();
  ...
  mod find {
    ...
    async fn returns_some_after_upsert() {
      let db = setup_db().await;
      let repo = Repo::new(&db);
      repo.upsert(&make_stargate(50001410, "Jita Gate")).await.unwrap();
      let result = repo.find(50001410).await.unwrap();
      assert!(result.is_some());
      assert_eq!(*result.unwrap().id(), 50001410);
    }
  }
  ...
  mod find_by_name {
    ...
    #[tokio::test]
    async fn returns_stargate_by_name() {
      let db = setup_db().await;
      let repo = Repo::new(&db);
      repo.upsert(&make_stargate(1, "Jita Gate")).await.unwrap();
      assert!(repo.find_by_name("Jita Gate").await.unwrap().is_some());
    }
  }
  ...
  mod upsert_many {
    ...
    #[tokio::test]
    async fn inserts_multiple_stargates() {
      let db = setup_db().await;
      let repo = Repo::new(&db);
      let records = vec![make_stargate(1, "Gate A"), make_stargate(2, "Gate B")];
