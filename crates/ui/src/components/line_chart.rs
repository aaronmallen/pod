...
impl<M> canvas::Program<M> for Component {
  ...
  fn draw(
    ...
    let grid_alpha = 0.06_f32;
    let grid_color = Color::from_rgba(0.957, 0.949, 0.925, grid_alpha);
    ...
    for i in 0..=4 {
      ...
      if !self.x_labels.is_empty() {
        ...
        frame.fill_text(Text {
          content: label,
          position: Point::new(w - self.pad_right - 4.0, gy - 3.0),
    ...
    let end_x = x_at(last_idx);
    let end_y = y_at(self.data[last_idx]);

    let end_dot = Path::circle(Point::new(end_x, end_y), 3.5);
    frame.fill(&end_dot, c);
