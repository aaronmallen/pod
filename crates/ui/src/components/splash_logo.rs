...
/// Builder for the animated Pod logo.
...
impl Component {
  ...
  pub fn render<MSG: 'static>(self) -> Element<'static, MSG> {
    let letterforms = svg(build_letterforms_svg(self.rotation, self.expand))
      .width(Length::Fill)
      .height(224.0)
      .style(|_, _| svg::Style {
    ...
    let dot = svg(build_dot_svg(self.pulse))
      .width(Length::Fill)
      .height(224.0)
      .style(|_, _| svg::Style {
...
fn build_letterforms_svg(rotation: f32, expand: f32) -> svg::Handle {
...
fn build_dot_svg(pulse: f32) -> svg::Handle {
