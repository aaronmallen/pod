...
fn contact_row<'a>(contact: &'a CharacterContact, is_last: bool) -> Element<'a, Message> {
  ...
  let standing_color = contact_standing_color(v);
  ...
  let label_text = contact.label_names.join(", ");
  let note_el = text(label_text)
    .font(body::REGULAR)
    .size(12.0)
    .style(|_: &Theme| iced::widget::text::Style {
      color: Some(color::text::SECONDARY),
  ...
  let watch_el: Element<'_, Message> = if contact.is_watched {
    container(
      text("WATCH")
        .font(mono::REGULAR)
        .size(8.5)
  ...
  let watch_container = container(watch_el)
    .width(80.0)
    .align_x(iced::alignment::Horizontal::Right);
