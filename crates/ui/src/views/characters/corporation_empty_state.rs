...
impl<'a> CorporationEmptyState<'a> {
  ...
  pub fn render<MSG: 'static>(self) -> Element<'a, MSG> {
    match self.query {
      None => corp_empty_placeholder(),
      Some(q) => corp_no_results(q),
    }
  }
}

fn corp_empty_placeholder<'a, MSG: 'static>() -> Element<'a, MSG> {
  container(
...
fn corp_no_results<'a, MSG: 'static>(q: &'a str) -> Element<'a, MSG> {
  container(
    column([
      text("No results")
