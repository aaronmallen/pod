...
use crate::{
  ...
  style::{color, typography::{body, mono}},
  ...
  if let Some(m) = mono_label {
    children.push(
      text(m)
        ...
        .size(10.0)
        .style(move |_: &Theme| iced::widget::text::Style {
          color: Some(if active { color::accent::PLASMA } else { color::text::SECONDARY }),
        })
        .into(),
    );
  }
...
impl<'a> Component<'a> {
  ...
  pub fn new(label: &'a str, active: bool, msg: Message) -> Self {
    Self { label, mono_label: None, active, msg }
  }
...
impl<'a> CharComponent<'a> {
  ...
  pub fn new(
    ...
    Self { name, tone, liquid, active, msg }
  }
