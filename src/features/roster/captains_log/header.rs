...
use crate::ui::{
  components::{button::Button, date_picker::DatePicker, icon::Icon},
...
pub(super) fn jump_calendar(state: &State) -> Element<'_, Parent> {
  ...
  let available: HashSet<String> = state.all_dates.iter().cloned().collect();
  ...
  DatePicker::new(
    ...
    move |date| {
      if date == today {
        ...
        Parent::Entries(entries::Message::Selected(Some(date.format("%Y-%m-%d").to_string())))
      }
    },
    Parent::Header(Message::PrevMonth),
    Parent::Header(Message::NextMonth),
  )
