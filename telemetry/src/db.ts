...
const INSERT_EVENT = `INSERT INTO events
  (anon_id, session, schema, app_version, git_sha,
    ...
    os, os_version, arch, window_size, screen_size, locale, app_language, event_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
export function buildStatements(db: D1Database, env: Envelope): D1PreparedStatement[] {
  ...
  if (env.kind === "session") {
      ...
      for (const ev of usage.events) {
        ...
        stmts.push(
          db.prepare(INSERT_EVENT).bind(
            ...
            null, null, null,
            null, null, null, null, null, null, null, ev.t,
          ),
        );
      }
    }
      ...
      for (const view of perf.views) {
        stmts.push(
          db.prepare(INSERT_EVENT).bind(
            ...
            null, null, null, null, null, null, null, env.sent_at,
          ),
        );
      }
    }
    ...
    if (e) {
      ...
      // app_language is null when an (old) client omits it.
      const appLanguage = e.app_language ?? null;
      stmts.push(
        db.prepare(INSERT_EVENT).bind(
          env.id, env.session, env.schema, env.app.version, gitSha,
          "environment", null, null, null,
          null, null, null,
          e.os, e.os_version, e.arch, windowSize, screenSize, e.locale, appLanguage, env.sent_at,
        ),
      );
    }
    return stmts;
